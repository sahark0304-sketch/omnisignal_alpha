...
class MacroFilter:
    ...
    def check_signal(
        ...
        if bias == "SHORT_ONLY" and action == "BUY":
            ...
            if get_trading_mode() in ("REDUCED", "BLOCKED"):
                return False, (
...
# ─────────────────────────────────────────────────────────────────────────────
#  MISSING IMPORT FIX (numpy needed for clip in compute_cot_bias)
# ─────────────────────────────────────────────────────────────────────────────
import numpy as np   # noqa: E402 — placed here intentionally for clarity
